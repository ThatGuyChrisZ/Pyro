...
def receive_and_decode_packets():
    ...
    try:
        rf_serial = serial.Serial(port='/dev/ttyUSB1', baudrate=9600, timeout=10, rtscts=True, dsrdtr=True)  # ADJUST PORT, BAUDRATE AS NECESSARY
    ...
    while True:
        try:
            ...
            payload = data[:-4]  # all but the checksum
            received_checksum = struct.unpack('<I', data[-4:])[0]  # last 4 bytes
            ...
            # Deserialize the payload into a Packet
            ...
            # Send the decoded packet to the server
